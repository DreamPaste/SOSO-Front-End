...
interface VotesBoardCommentListProps {
  pollId: number;
  initialCount?: number;
}

export default function VotesBoardCommentList({
  pollId,
  initialCount,
}: VotesBoardCommentListProps) {
  ...
  const comments = useMemo<
    Array<PollCommentSummary & { commentId: number }>
  >(() => {
    ...
    return allComments.filter(
      (
        comment,
      ): comment is PollCommentSummary & { commentId: number } =>
  ...
  return (
    <section aria-label="댓글 섹션" className="flex-1">
      <h2 id="poll-comments-heading" className="sr-only">
        댓글
      </h2>
      ...
      <div
        role="region"
        aria-labelledby="poll-comments-heading"
      ...
      >
        ...
        >
          ...
          <InfiniteScroll.Contents
            ...
            scrollStore={{
              enabled: true,
              storageKey: `poll-comment-scroll-${pollId}`,
              resetScroll: false,
            }}
            getItemKey={(
              comment: PollCommentSummary & { commentId: number },
            ) => comment.commentId}
            renderItem={(comment) => (
              <PollCommentItem comment={comment} />
            )}
            gap={8}
            threshold={0.6}
          >
